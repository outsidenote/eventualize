...
Each round node that use saw above is a folding logic, and each box after that node is the resulting state.

At this points you probably have some questions.
Like: 
...
There are several mechanisms that needs to be put in place in order for event sourcing to work not only in principal but also in real life. This is why we have built `Eventualize` - so you wouldn't have to!
