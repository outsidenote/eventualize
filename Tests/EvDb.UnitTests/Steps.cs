...
internal static class Steps
{
        ...
        ITestOutputHelper output)
    {
        ...
        A.CallTo(() => storageAdapter.StoreStreamAsync(A<IImmutableList<EvDbEvent>>.Ignored, A<IImmutableList<EvDbMessage>>.Ignored, A<CancellationToken>.Ignored))
                .Throws<OCCException>();
        ...
        long initOffset = 0)
    {
        ...
        if (withEnlisted)
        {
            EvDbStreamCursor cursor = new(factory.StreamType, streamId, initOffset);
        ...
        for (int i = 0; i < 3; i++)
        {
            EvDbStreamCursor cursor = new(factory.StreamType, streamId, initOffset + i);
        ...
        Func<int, double>? gradeStrategy = null)
    {
        ...
        var result = await stream.GivenAddGrades(testId, studentId, numOfGrades, gradeStrategy);
        return result;
    }

    public static async Task<IEvDbSchoolStream> GivenAddGrades(this IEvDbSchoolStream stream,
        int testId = TEST_ID,
        int studentId = STUDENT_ID,
        int numOfGrades = NUM_OF_GRADES,
        Func<int, double>? gradeStrategy = null) =>
           await stream.WhenAddGrades(testId, studentId, numOfGrades, gradeStrategy);
